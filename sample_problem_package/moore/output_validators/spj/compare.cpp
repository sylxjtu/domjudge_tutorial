...
int main(int argc, char **argv) {
    ...
    // HACK: Use judge in to replace judge ans
	openfile(judgeans, argv[1], argv[0]);
	...
	while (true) {
		...
		if (use_floats && isfloat(judge.c_str(), jval)) {
            ...
            if(!spj(team, stoi(judge), errorMessage)) {
				wrong_answer("SPJ returns: %s\nJudge: \"%s\"\nTeam: \"%s\"", errorMessage.c_str(), judge.c_str(), team.c_str());
            }
